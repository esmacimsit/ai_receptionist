...
def transcribe_audio(audio_np):
    ...
    if USE_MLX:
        ...
        segments, _ = fw_model.transcribe(audio_np, language="tr", beam_size=1)
        text = "".join(s.text for s in segments).strip()

    # --- SECURITY FIREWALL ---
    text_lower = text.lower()
    
    # 1. HALLUCINATION FILTER (Sessiz)
    yasakli_listesi = [
        "abone ol", "beğenmeyi", "bildirimleri", "kanalımıza", 
        "izlediğiniz için", "thank you", "altyazı", "subtitle", 
        "devam edecek", "copyright", "konuşma devam ediyor"
    ]
